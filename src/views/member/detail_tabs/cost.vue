        ...
        <el-tabs v-model="activeName" v-on:tab-click="handleClick">
            <el-tab-pane label="团课消费" name="tuanke"> 
                ...
                <el-table :data="tuankeList" border :header-cell-style="{'color':'#333', 'background-color':'#f5f5f5'}">
                    ...
                    <el-table-column prop="name" width="150" label="用户"></el-table-column>  
                    <el-table-column prop="card_name" width="180" label="卡名称"></el-table-column>   
                    ...
                    <el-table-column width="120" label="卡内剩余">
                        <template slot-scope="scope">
                            <span v-if="scope.row.cost_type == 1">{{scope.row.surplus}} 次</span>
                            <span v-else-if="scope.row.cost_type == 6">{{scope.row.surplus.toFixed(2)}} 元</span>
                            <span v-else-if="scope.row.cost_type == 7">{{scope.row.surplus}} 小时</span>
                            <span v-else></span>
                        </template>
                    </el-table-column>
                    <el-table-column prop="order_remark" label="备注"></el-table-column>
            ...
            <el-tab-pane label="私教课消费" name="sijiao">
                ...
                <el-table :data="sijiaoList" border :header-cell-style="{'color':'#333', 'background-color':'#f5f5f5'}">
                    ...
                    <el-table-column width="180" label="确认时间">
                        <template slot-scope="scope">
                            <span>{{scope.row.order_time && scope.row.order_time.slice(0, 16)}}</span>
                        </template>
                    </el-table-column>  
